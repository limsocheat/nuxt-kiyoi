	...
	<v-app class="mx-5 my-5">
		...
		<div class="d-flex justify-space-between">
			...
			<div class="print">
				<a
					class="print--link"
					:href="baseURL + `api/department/export_pdf`"
				>
					<v-btn
						dark
						class="red darken-1"
				...
				<a
					class="print--link"
					:href="baseURL + `api/department/export`"
				>
					<v-btn
						dark
		...
		methods: {
			...
			deleteItem(item) {
				if (confirm("Are u sure to delete it?")) {
					this.$axios
						.$delete(`/api/hr-department/` + item.id)
						.then(res => {
							this.fetchData();
			...
			createItem() {
				if (this.editedIndex > -1) {
					this.$axios
						.$patch(`/api/hr-department/` + this.form.id, {
							name: this.form.name
						})
						.then(res => {
							this.fetchData();
					...
					this.$axios
						.$post(`/api/hr-department`, this.form)
						.then(res => {
							this.form = res.data;
							this.fetchData();
